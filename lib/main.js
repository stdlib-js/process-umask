...
function umask() {
	...
	if ( nargs === 1 ) {
		...
		if ( isObject( arg ) ) {
			if ( hasOwnProp( arg, 'symbolic' ) ) {
				...
				if ( !isBoolean( opts.symbolic ) ) {
					throw new TypeError( format( '0nl2o', 'symbolic', opts.symbolic ) );
				}
			}
			mask = proc.umask();
			if ( opts.symbolic ) {
		...
		throw new TypeError( format( '0nl6r', arg ) );
	}
	options = arguments[ 1 ];
	if ( !isObject( options ) ) {
		throw new TypeError( format( '0nl2V', options ) );
	}
	...
	if ( isString( arg ) ) {
		...
		throw new TypeError( format( '0nl6s', arg ) );
	}
	// Set the mask:
