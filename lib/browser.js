...
function umask() {
	...
	if ( nargs === 1 ) {
		...
		if ( isObject( arg ) ) {
			if ( hasOwnProp( arg, 'symbolic' ) ) {
				...
				if ( !isBoolean( opts.symbolic ) ) {
					throw new TypeError( format( '0nl2o,GE', 'symbolic', opts.symbolic ) );
				}
			}
			mask = MASK;
			if ( opts.symbolic ) {
		...
		throw new TypeError( format( '0nl6r,GF', arg ) );
	}
	options = arguments[ 1 ];
	if ( !isObject( options ) ) {
		throw new TypeError( format( '0nl2V,FD', options ) );
	}
	...
	if ( isString( arg ) ) {
		...
		throw new TypeError( format( '0nl6s,GG', arg ) );
	}
	// Set the mask:
	mask = MASK;
